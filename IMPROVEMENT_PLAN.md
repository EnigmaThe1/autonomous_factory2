...
- [x] **Phase 12 (agent capabilities)** — `myAi.openMcpConfig` command; AGENT_CAPABILITIES Phase 2a `servers` JSON example aligned with host; **Chat & Missions** view title entry (JSON icon)
...
- [x] **Commands** — `myAi.approveMissionBlueprint`, `rejectMissionBlueprint`, `requestMissionBlueprintRevision`, **`openMissionAutonomyBlueprint`** (Command Palette + **Chat & Missions** view title book), **`myAi.openMissionSettings`** (VS Code Settings → `myAi.missions`; Palette + view title gear), **`myAi.exportMissionBlueprint`**, **`myAi.copyMissionBlueprint`** (markdown + pre-Q&A to clipboard or file).
