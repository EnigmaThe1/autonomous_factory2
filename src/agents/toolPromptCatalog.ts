...
function lazyToolLines(): string[] {
  return [
    "LAZY TOOL CATALOG: only common tools are listed below. Emit TOOL:{\"tool\":\"listTools\",\"args\":{}} for built-in names, external adapters, `mcpServers` (names from MCP JSON, no process spawn), and bounded `hints` (myAi.tools.listToolsHintsMaxChars). Emit TOOL:{\"tool\":\"listMcpTools\",\"args\":{}} for MCP tools (set myAi.tools.listMcpToolsSummaryMaxChars > 0 for summaries with inputPropertyNames, no full inputSchema), then call mcp.server_name.tool_name.",
    'TOOL:{"tool":"readFile","args":{"path":"..."}}',
