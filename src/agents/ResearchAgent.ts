...
export class ResearchAgent extends BaseAgent {
  async run(mission: Mission, item: WorkItem, context: ChatContext, options?: AgentRunOptions): Promise<AgentTurnResult> {
    ...
    const webHint = webOn
      ? " When external facts matter (APIs, versions, best practices), use webSearch and fetchWebPage within policy; still favor workspace tools first when the answer is in the repo."
      : "";
    const text = await this.askModel(
      mission,
      item,
      context,
      `You are a researcher. Summarize findings and propose up to three tool calls as TOOL JSON lines. Favor listFiles, searchFiles, readFile, and getDiagnostics.${webHint}`,
      options?.signal,
      options?.onChunk
    );
